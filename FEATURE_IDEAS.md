...
- **Technical Challenge (Extreme):** Normal PDFs are fundamentally structured like a *painting/canvas*, not a Word document. A line of text might not even be a "string" but a bunch of drawn vector lines or disconnected glyphs. Syncfusion does *not* natively provide a `.removeText()` function for existing flattened graphics. 
  - *Approach A (The Tipe-X Method)*: Painting a "White Box" directly over the coordinate to hide it visually. However, this has a massive flaw: if the PDF has a colored background or texture, a white box will look horribly misplaced. This method would require building an intelligent "Color Picker/Eyedropper" so the eraser box can perfectly chameleon into its surrounding pixels.
    - *Hybrid Engine Strategy*: To properly power this bounding box detection, we will need a dual-engine architecture:
      1. **Primary/Fast Engine**: Native `PdfTextExtractor` (Syncfusion) - Instantly accurate and lightweight, but only works on digital/exported PDFs.
      2. **Fallback/Pro Engine**: `google_ml_kit_text_recognition` - A user-toggled AI scanner specifically built to detect text on Scanned Images/Photos/Stamps inside PDFs where the Native engine is inherently blind.
  - *Approach B (The Hardcore Byte Parser)*: Parsing the raw byte code (`content stream`) of the PDF to rip out the drawing commands. Highly volatile and prone to corrupting the file structure.
